...
parser.add_option("-c", "--coverage-threshold", dest="c",
                  help="a list containing the minimum and the maximum coverage thresholds separated by a comma: e.g. 10,80")
parser.add_option("-b", "--base-quality-threshold", dest="b",
                  help="The Base-quality threshold for Qualities encoded in Sanger format (Illumina 1.8 format)")
parser.add_option("-u", "--cutoff", dest="u",
                  help="output-file: Do not add any extension (such as .fasta) as the program will do this automatically")
...
if len(list(map(int, options.c.split(",")))) > 2:
    covmin = map(int, options.c.split(","))[0]
    covmax = map(int, options.c.split(","))[1:]
else:
    covmin, covmax = list(map(int, options.c.split(",")))

##################################### main code #################################################

# parse mpileup and store alternative alleles:
allelehash = collections.defaultdict(
