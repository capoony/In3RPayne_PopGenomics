...
def AfHf(x, y):
    ...
    a = x.replace("N", "")[0]
    b = y.replace("N", "")[0]
    if len(list(zip(*[w for w in zip(x, y) if not "N" in w]))) < 2:
        return "NA"
...
START, END = list(map(int, Positions.split("-")))
...
SH = d(list)
if options.sa != "NA":
    for k, v in list(SsH.items()):
        SH[k] = random.sample(v, int(options.sa))
else:
    MinLen = min([len(x) for x in list(SsH.values())])
    for k, v in list(SsH.items()):
...
for i in NAMES:
    c = 1
    PP = random.sample(list(PH[i].keys()), int(options.sn))
    for P1 in PP:
        for P2 in PP:
...
RH1 = d(list)
for i in NAMES:
    A = random.sample(list(range(len(RH[i]))), int(options.c))
