...
if "/" not in options.a:
    ...
    for i in range(len(coverage.split("+"))):
        ...
        covdict[i+3] = dict(zip(head,
                            map(int, coverage.split("+")[i].split(","))))
# mincov
# print covdict
############################### determine haplotype #########################################
...
for line in open(options.input, "r"):
    ...
    # print "N's"	and exit, if reference is ambiguous
        ...
        out3.write("\t".join(a[:3])+"\t"+"/".join(reference.keys()) +
    ...
    print "\t".join(a[:3])+"\t"+"/".join(refal)+"\t"+"".join(hapallele)+"\t"+"".join(qual)+"\t"+"/".join(fullalleles.keys())
    # out.flush()
